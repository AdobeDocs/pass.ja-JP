...
MVPD M は、iPhoneアプリケーション、iPadアプリケーションおよび Web サイトを有する。 プログラマー P のリクエストに応じて、Adobe同時実行性モニタリング（CM）と統合する必要があります。プログラマー P は、同時使用を制限するルールを含んだ一連のポリシーを CM で定義しました。 CM は、定義されたポリシーとルールに基づいて、ストリームの再生を許可する決定を行います。 つまり、ストリームの開始と再生が許可されていても、再生中に CM がストリームの停止を決定できます。
...
| ストリームコンプライアンスの間隔 | Adobe同時実行監視に対するストリームのコンプライアンスをチェックする間隔（秒） | 180 | 180 |
