---
title: 単一のテナント/ポリシーおよび複数のアプリケーションに対するCMの実装
description: 単一のテナント/ポリシーおよび複数のアプリケーションに対するCMの実装
...
# 単一のテナント/ポリシーおよび複数のアプリケーションに対するCMの実装 {#imp-cm}



## ユースケース

プログラマーPは、iPhoneアプリケーション、iPadアプリケーション、およびweb サイトを持っています。 It部門は、Adobe Concurency Monitoring （CM）と統合して、アプリ間の同時ストリーム数を制限する必要があります。 プログラマは、同時使用を制限するポリシーを作成します。 2つの例を見てみましょう。

最初のポリシーには、2つ以上の同時ストリームを許可しないルールが1つ含まれています。 最新のストリームが再生できるようになります。
2つ目のポリシーには2つのルールが含まれています。 2台以下のデバイスから3つ以上の同時ストリームを使用できず、最新のストリームを再生できます。


## 1つのルールを持つポリシー

最初のポリシーには、1つのルールが含まれています。2つ以上の同時ストリームは許可されていません。3つの同時ストリームが再生されている場合、最初に開始するストリームは停止されます。

2つのアプリと1つのweb サイトを使用して、ストリームを開始します。

1. ユーザーは、iPhone アプリから1つのストリームを開始し、iPad アプリから1つのストリームを開始します。 このポリシーはそれを許可しています。
1. その後、ユーザーはプログラマーのウェブサイトから3番目のストリームを開始します。
1. ポリシーのルール（最大2 ストリーム、最新の勝者）では、最新のストリームを再生できるので、**開始された最初のストリームは、ポリシーに準拠していないとマークされ、停止されます。**
