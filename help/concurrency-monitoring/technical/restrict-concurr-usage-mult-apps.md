---
title: 異なる所有者に属する複数のアプリケーションの同時使用を制限します
description: 異なる所有者に属する複数のアプリケーションの同時使用を制限する
...
# 異なる所有者に属する複数のアプリケーションの同時使用を制限する {#restr-concurr-usage}


## ユースケースの説明

MVPD M には、iPhone アプリケーション、iPad アプリケーションおよび web サイトがあります。 プログラマー P のリクエストに応じて、Adobe Concurrency Monitoring （CM）と統合する必要があります。プログラマー P は、同時使用を制限するルールを含んだ一連のポリシーを CM で定義しました。 CM は、定義されたポリシーとルールに基づいて、ストリームの再生を許可する決定を行います。 つまり、ストリームの開始と再生が許可されていても、再生中に CM がストリームの停止を決定できます。
...
1. 会社の名前
1. cm と統合するアプリケーション。 アプリケーションごとに、次の情報を指定する必要があります。
   - アプリケーション名
   - アプリケーションプラットフォーム
1. 統合をリクエストしたサードパーティ（該当する場合）


チケット作成後、次の情報が使用するためにリリースされます。

| タイプ | 説明 | 値の例 | デフォルト値 |
| --- | --- | --- | --- |
| endpoint | Adobe同時実行性モニタリングのエンドポイント | http://streams.adobeprimetime.com/v1/ | http://streams.adobeprimetime.com/v1/ |
| applicationId | iPhone アプリ ID | iphone54-75b4-431b-adb2-eb6b9e546013 | - |
| applicationId | iPad アプリ ID | ipad5d54-75b4-431b-adb2-eb6b9e546013 | - |
| applicationId | web サイトアプリ ID | website4-75b4-431b-adb2-eb6b9e546013 | - |
| 心拍間隔 | Adobe同時実行監視にハートビート呼び出しを送信する間隔（秒） | 60 | 60 |
| ストリームコンプライアンスの間隔 | Adobe同時実行監視へのストリームのコンプライアンスをチェックする間隔（秒） | 180 | 180 |


## 実装のガイドライン

以下の項目をアプリケーションにパッケージ化する必要があります。
