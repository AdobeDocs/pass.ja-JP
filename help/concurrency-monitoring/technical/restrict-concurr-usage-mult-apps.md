...
MVPD Mには、iPhone アプリケーション、iPad アプリケーション、およびweb サイトがあります。 プログラマーPのリクエストにより、Adobe同時視聴数モニタリング（CM）と統合する必要があります。プログラマーPは、CMで、同時使用を制限するルールを持つ一連のポリシーを定義しています。 CMは、定義されたポリシーとルールに基づいて、どのストリームを再生できるかを決定します。 つまり、ストリームの開始と再生が許可されていても、再生中にCMはそれを停止できます。
...
CMと統合するには、Zendesk チケット（https://adobeprimetime.zendesk.com）を作成し、次の情報を指定する必要があります。

1. 会社の名前
1. cmsと統合するアプリケーション。 各アプリケーションについて、次を提供する必要があります。
   - アプリケーション名
   - アプリケーションプラットフォーム
1. 統合を要求した第三者（該当する場合）


チケットを作成すると、次の情報が公開され、使用できるようになります。

| タイプ | description | 値の例 | デフォルト値 |
| --- | --- | --- | --- |
| エンドポイント | Adobe同時視聴数モニタリングのエンドポイント | http://streams.adobeprimetime.com/v1/ | http://streams.adobeprimetime.com/v1/ |
...
| 心拍数の間隔 | Adobe Concurrency Monitoringにハートビート呼び出しを送信する間隔（秒単位） | 60 | 60 |
| ストリーム準拠の間隔 | Adobe同時視聴数モニタリングのストリームコンプライアンスをチェックするための秒単位のインターバル | 180 | 180 |


## Adobe Analyticsの実装ガイドライン

次の項目は、アプリケーションにパッケージ化する必要があります。
