...
現在、デフォルトの制限では、1 秒あたり最大 1 件のリクエストが許可されています。初期のバーストは 3 件のリクエストです（識別されたクライアントの最初のインタラクションに対する 1 回限りの許可。これにより、初期化が正常に終了します）。 これは、すべての顧客の通常のビジネスケースには影響しません。
...
| Second 1.4 | コールが 429 ステータス コードを受け取る | 制限から消費された呼び出しが 2 件、バーストとしてマークされた呼び出しが 3 件、「429 Too many requests」が 1 件の呼び出しを受け取ります |
| Second 1.6 | コールが 429 ステータス コードを受け取る | 制限から消費された 2 件の呼び出し、バーストとしてマークされた 3 件の呼び出し、および 2 件の呼び出しは「429 Too many requests」を受信 |
| Second 1.8 | コールが 429 ステータス コードを受け取る | 制限から消費された呼び出しが 2 回、バーストとしてマークされた呼び出しが 3 回、「429 Too many requests」が 3 回 |
| Second 2.1 | 呼び出しが成功ステータスコードを受信します | 制限から消費された呼び出しが 3 件、バーストとしてマークされた呼び出しが 3 件、「429 Too many requests」が 3 件となります |
