...
* **特定のコードのプロファイルを取得：** ストリーミングアプリケーションは、`code`/api/v2/[**/profiles/code/{serviceProvider} エンドポイントを呼び出してプロファイルが正常に生成され保存されたかどうかを確認するために、{code}**](/help/authentication/integration-guide-programmers/rest-apis/rest-api-v2/apis/profiles-apis/rest-api-v2-profiles-apis-retrieve-profile-for-specific-code.md) を使用してポーリングメカニズムを実装する必要があります。
