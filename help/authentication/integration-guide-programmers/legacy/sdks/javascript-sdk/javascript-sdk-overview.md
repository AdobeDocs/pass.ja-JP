...
一般的なAdobe Pass認証の使用権限のフローについては [ プログラマーの使用権限のフロー ](/help/authentication/integration-guide-programmers/entitlement-flow.md) を参照し、JavaScript統合クックブックで実装手順を説明します。 次のセクションでは、JavaScript AccessEnabler の統合に関する説明とサンプルを示します。
