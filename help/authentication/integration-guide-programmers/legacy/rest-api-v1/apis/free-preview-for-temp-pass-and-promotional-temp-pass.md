...
| _deviceType_ | デバイスタイプ（Roku、PC など）。</br></br> このパラメーターが正しく設定されている場合、ESM では、Roku、AppleTV、Xbox などのさまざまなタイプの分析を実行できるように、クライアントレス使用時に [ デバイスタイプごとに分類 ](/help/authentication/integration-guide-programmers/features-premium/esm/entitlement-service-monitoring-overview.md#clientless_device_type) された指標を提供します。</br></br>[ クライアントレスデバイスタイプのパラメーターを使用するメリット ](/help/authentication/integration-guide-programmers/legacy/notes-technical/benefits-of-using-the-clientless-devicetype-parameter-in-pass-metrics.md)</br></br>**メモ**:device_info は、このパラメーターを置き換えます。 |
