...
| ユーザーアクション | システムアクション |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <br/><br/>MVPDピッカーが表示されます。<br/><br/>MVPDを選択して、ログインを続行します。 | バックグラウンドチェックは、MVPDがユーザーの識別にルールを適用する際に実行されます。 たとえば、ユーザの IP アドレスを、ディストリビュータがプロビジョニングしたモデムまたはブロードバンド接続されたセットトップ ボックスのMAC アドレスにマッピングする場合があります。 |
| 約 3 秒間保持される画面が表示されます。<br/><br/> インタースティシャルページは、MVPD アカウントを使用して自動ログインしていることをユーザーに通知する場合があります。 | <br/><br/>Adobe Pass認証エンドポイントは、Adobe Pass リダイレクトを介してリクエストをMVPD認証エンドポイントに転送します。<br/><br/>MVPDは、「true」または「false」の値を持つ HBA フラグ（`hba_status`）を含む SAML レスポンスの形式で認証決定を送信することが想定されます。<br/><br/>Adobe Pass Authentication バックエンドは、MVPD ユーザープロファイル エンドポイントにリクエストを行い、`hba_status` フラグを [&#x200B; ユーザーメタデータ &#x200B;](/help/authentication/integration-guide-programmers/features-standard/entitlements/user-metadata.md) の一部として公開します。 |
...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <br/><br/>MVPDピッカーが表示されます。<br/><br/>MVPDを選択して、ログインを続行します。 | バックグラウンドチェックは、MVPDがユーザーの識別にルールを適用する際に実行されます。 たとえば、ユーザの IP アドレスを、ディストリビュータがプロビジョニングしたモデムまたはブロードバンド接続されたセットトップ ボックスのMAC アドレスにマッピングする場合があります。 |
| 約 3 秒間保持される画面が表示されます。<br/><br/> インタースティシャルページは、MVPD アカウントを使用して自動ログインしていることをユーザーに通知する場合があります。 | <br/><br/><br/><br/> MVPD認証エンドポイントは、Adobe Pass認証エンドポイントに認証コードを送信します。<br/><br/>Adobe PassMVPD認証は、Adobe PassAdobe Pass MVPD認証エンドポイントから更新トークンとアクセストークンをリクエストするために認証コードを使用します。<br/><br/>MVPDは、`id_token` の一部として「true」または「false」の値を持つ HBA フラグ（`hba_status`）を含む認証判定を送信する必要があります。<br/><br/>Adobe Pass Authentication バックエンドがMVPDにリクエストします。`hba_status` フラグを [&#x200B; ユーザーメタデータ &#x200B;](/help/authentication/integration-guide-programmers/features-standard/entitlements/user-metadata.md) の一部として公開するプロファイルエンドポイント。<br/><br/>MVPDは、更新トークン TTL をMVPDで合意された値に設定し、Adobeは、認証 TTL を更新トークンの値以下の値に設定します。 |
| ユーザーが認証され、タイトルの付いた TV Everywhere コンテンツを参照できるようになりました。 | アプリケーションはユーザープロファイルを取得し、さらに決定フローを進めてコンテンツを再生できます。 |
