...
Temp Pass を使用すると、MVPDのアカウント資格情報を持たないユーザーに対して、保護されたコンテンツへの一時的なアクセスを提供できます。  一時パスには、次の機能が含まれます。
...
* プログラマーは、特定のリクエスターに付与されたすべてのトークンをリセットできます。  一時パスの実装に使用する「一時MVPD」は、「リクエスターごとの認証」が有効になっている必要があります。
* **特定のデバイスの個々のユーザーに一時パスアクセスが付与されます**。 ユーザーの Temp Pass アクセスの有効期限が切れると、そのユーザーの有効期限切れの認証トークンがAdobe Pass認証サーバーからクリアされるまで、そのユーザーは同じデバイスで一時的にアクセスできなくなります。
...
* **設定パラメーター** – 一時パスルールを作成するために、プログラマーが次の一時パスパラメーターを指定できます。
   * **トークン TTL** - MVPDにログインせずにユーザーが視聴できる時間。 この時間は時計ベースで、ユーザーがコンテンツを視聴しているかどうかに関係なく期限切れになります。
  >[!NOTE]
  >要求者 ID に複数の一時パス ルールを関連付けることはできません。
* **Authentication / Authorization** – 一時パスフローでは、MVPDを「一時パス」として指定します。  Adobe Pass認証は、一時パスフローの実際のMVPDと通信しないので、「一時パス」MVPDがすべてのリソースを認証します。 プログラマーは、サイトの残りのリソースと同様に、一時パスを使用してアクセス可能なリソースを指定できます。 メディア検証者ライブラリは、通常どおり使用して、一時パスの短いメディアトークンを検証し、再生前にリソースのチェックを実施できます。
* **一時パスフローでのトラッキングデータ** – 一時パス使用権限フロー中のトラッキングデータに関する 2 つの点：
   ...
   * 一時パスフローで使用されるMVPD ID は「一時パス」なので、同じMVPD ID が **sendTrackingData （）** に返されます。 ほとんどのプログラマーは、Temp Pass 指標を実際のMVPD指標とは異なる方法で扱う必要があるでしょう。 これには、Analytics の実装で追加の作業が必要です。
...
Adobe Pass認証側では、「TempPass」という擬似MVPDを実装し、参加するプログラマのサーバ構成に実装します。  この疑似MVPDは、プログラマーの保護されたコンテンツへのアクセスを一時的に許可する、実際のMVPDのように機能します。

プログラマー側では、MVPD が認証に使用する 2 つのシナリオに対して、一時パスが次のように実装されます。

* **プログラマーのページの iFrame**。 一時パスはMVPDの認証タイプに関係なく機能しますが、iFrame シナリオの場合、現在の認証フローをキャンセルして一時パスによる認証を行うには、さらに手順が必要です。 これらの手順については、以下の [iFrame ログイン ](/help/authentication/integration-guide-programmers/features-premium/temporary-access/temp-pass.md) を参照してください。
* **MVPD ログインページにリダイレクトし直します**。 より従来の場合、MVPDで認証を開始する前に Temp パスをトリガーする UI が表示されるので、特別な手順はありません。 Temp Pass は通常のMVPDと同様に扱われます。

両方の実装シナリオに次の点が適用されます。

* 「一時パス」は、まだ一時パスの認証をリクエストしていないユーザーに対してのみ、MVPD ピッカーに表示されます。 以降のリクエストに対する表示をブロックするには、cookie にフラグを設定します。 これは、ユーザーがブラウザーのキャッシュをクリアしない限り機能します。 ユーザーがブラウザーのキャッシュをクリアすると、ピッカーに「一時パス」が再び表示され、ユーザーは再びリクエストできるようになります。 「一時パス」の期限がまだ切れていない場合にのみ、アクセスが許可されます。
...
* Temp Pass フローでは、認証トークンと認証トークンの有効期限が同じであるため、Temp Pass の有効期限が切れると、そのユーザーは認証されなくなります。 Temp Pass の有効期限が切れたことをユーザーに説明するために、プログラマーは `setRequestor()` を呼び出した直後に選択したMVPDを取得し、通常どおり `checkAuthentication()` を呼び出す必要があります。 `setAuthenticationStatus()` コールバックでは、認証ステータスが 0 であるかどうかをチェックできます。これにより、選択したMVPDが「TempPass」の場合、Temp Pass セッションの有効期限が切れたことを示すメッセージをユーザーに表示できます。
...
1. MVPDピッカーが開き、リストからMVPDを選択できます。
...
以下のサンプルは、ユーザーがサイトにアクセスしたときに TempPass で自動的にログインするケースを示しています。 ユーザーはいつでも通常のMVPDでログインすることができ、TempPass の有効期限が切れると警告されます。
...
このサンプルシナリオ（最初の 4 時間のフリーセッション、その後の 1 日 10 分のフリーセッション）の場合、Adobeでは、TempPass1 というMVPDで 4 時間の Time-To-Live （TTL）を設定し、TempPass2 で 10 分の TTL を設定して、それ以降の期間に適用します。  どちらも、プログラマーのリクエスター ID に関連付けられています。

### プログラマ実装 {#mult-tempass-prog-impl}

Adobeが 2 つの TempPass インスタンスを設定すると、追加の 2 つの MVPD （TempPass1 と TempPass2）がプログラマのMVPD リストに表示されます。  複数の一時パスを実装するには、プログラマーが次の手順を実行する必要があります。

1. ユーザーのサイトへの初回訪問時に、TempPass1 で自動的にログインします。 上記の自動ログインサンプルを、このタスクの出発点として使用できます。
1. TempPass1 の有効期限が切れていることが判明したら、その事実を（cookie/ローカルストレージに）保存し、標準のMVPD ピッカーを使用して表示します。 **そのリストから TempPass1 と TempPass2 を除外してください**。
1. TempPass1 の有効期限が切れた場合は、そのユーザーを TempPass2 で自動ログインします。
1. TempPass2 の有効期限が切れた場合は、その日のファクトを（cookie/ローカルストレージに）保存し、標準のMVPD ピッカーを使用して表示します。 ここでも、そのリストから TempPass1 と TempPass2 を除外してください。
...
一部のビジネス・ルールでは、Temp パスの定期的なパージ、または特定のリクエスタ ID とMVPDID に対して発行されたすべての Temp パスのアドホック・リセットが必要です。 この機能では、次のようなユースケースをサポートしています。
...
**JavaScript SDK**: バージョン **3.X 以降** の一時パスのリセット機能がサポートされています。
